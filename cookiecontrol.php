...
//  defaults
$def_cookiecontrol_logConsent = 'NO';
...
$def_cookiecontrol_alertBackground = '#eaeaea';
...
<?php global $cookiecontrol_settings; $options = $cookiecontrol_settings;?>
    ...
    <table class="form-table">
      ...
      <tr>
        ...
        <td>
          <input type="radio" class="first" name="cookiecontrol_settings[logConsent]" value="NO" <?php checked('NO', ( isset($options['logConsent']) ? $options['logConsent'] : '' )); ?> /><?php _e('No', 'cookie-control'); ?>
          <input type="radio" name="cookiecontrol_settings[logConsent]" value="YES" <?php checked('YES', ( isset($options['logConsent']) ? $options['logConsent'] : ' ' )); ?> /><?php _e('Yes', 'cookie-control'); ?>
        </td>
      </tr>
    </table>
...
ga('send', 'pageview');
            ...
            <tr>
              <th scope="row">
                <label for="cookiecontrol_settings[optionalCookiesinitialConsentState]"><?php _e('Initial Consent State', 'cookie-control'); ?></label>
                <p>If set to "on", cookies will be allowed by default for this category. </p>
              </th>
              <td>
...
ga('send', 'pageview');</pre>	
                                ...
                                <tr>
                                  <th scope="row">
                                    <label for="cookiecontrol_settings[optionalCookiesinitialConsentState][<?php echo $key ?>]"><?php _e('Initial Consent State', 'cookie-control'); ?></label>
                                    <p>If set to "on", cookies will be allowed by default for this category. </p>
                                  </th>
                                  <td>
...
function cookiecontrol_settings_validate($input) {
    ...
    if (!empty($input['altLanguages']) && is_array($input['altLanguages'])) {
      foreach ( $input['altLanguages'] as $key => $val ) :
        ...
        $input['altLanguagesText'][$key] = wp_kses($input['altLanguagesText'][$key]); 
      endforeach;
    }
...
function cookiecontrol_scripts() {
    if(!is_admin()) {
        wp_enqueue_script('cookiecontrol', '//cc.cdn.civiccomputing.com/8.0/cookieControl-8.0.min.js', array('jquery'), '', true);
    }
}
...
<script type="text/javascript">
  var config = {
    ...
    <?php if ( sizeof($cookiecontrol_settings['excludedCountries']) > 0 && !empty($cookiecontrol_settings['excludedCountries']) ) : ?>
      <?php 
    ...
    <?php if ( sizeof($cookiecontrol_settings['altLanguages']) > 0 && !empty($cookiecontrol_settings['altLanguages']) ) : ?>
    locales: [
    ...
    <?php
      $necessaryCookiesVal = "'wordpress_*','wordpress_logged_in_*',";
    ?>
        
    <?php if ( sizeof($cookiecontrol_settings['necessaryCookies']) > 0 && !empty($cookiecontrol_settings['necessaryCookies']) ) : ?>
      <?php 
    ...
    <?php if ( sizeof($cookiecontrol_settings['optionalCookiesName']) > 0 && !empty($cookiecontrol_settings['optionalCookiesName']) ) : ?>
    optionalCookies: [
      <?php foreach ( $cookiecontrol_settings['optionalCookiesName'] as $key => $val ) : ?>
          <?php if ( $val != '' ) : ?>
          {
            ...
            <?php endif; ?> 
            initialConsentState : '<?php echo $cookiecontrol_settings['optionalCookiesinitialConsentState'][$key]; ?>',
          },
          <?php endif; ?>
      <?php endforeach; ?>
    ],
